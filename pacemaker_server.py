...
# Set up the Lock Button with proper debounce timing
lock_button = Button(17, bounce_time=0.05)  # Reduced bounce time for faster response
...
# Lock state and button handling variables
is_locked = False
last_button_press_time = 0
lock_button_cooldown = 0.3  # seconds
lock_button_lock = threading.Lock()
...
# Improved toggle_lock function with debouncing and optional force_state
def toggle_lock(force_state=None):
    global is_locked, last_button_press_time
    
    # Get current time
    current_time = time.time()
    
    # Use a lock to prevent race conditions
    with lock_button_lock:
        # Check if we're still in cooldown period
        if current_time - last_button_press_time < lock_button_cooldown:
            print("Button press ignored (cooldown)")
            return
        
        # Update last press time
        last_button_press_time = current_time
        
        # If force_state is provided, use that instead of toggling
        if force_state is not None:
            is_locked = force_state
        else:
            is_locked = not is_locked
...
def on_button_release():
    ...
    toggle_lock()  # Toggle on release

# Attach event listeners
rate_encoder.when_rotated = update_rate
a_output_encoder.when_rotated = update_a_output
v_output_encoder.when_rotated = update_v_output
lock_button.when_pressed = on_button_press
lock_button.when_released = on_button_release
...
def set_lock():
    data = request.json
    force_state = data.get('state') if data else None
    toggle_lock(force_state)
    return jsonify({'success': True, 'locked': is_locked})
