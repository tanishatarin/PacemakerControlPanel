...
const ControlPanel: React.FC = () => {
  ...
  const lockStateChangingRef = useRef(false); // Prevent lock state fighting
  const lastLockToggleRef = useRef(0); // Prevent rapid toggling
  ...
  // Enhanced auto-lock timer management with better activity tracking
  const resetAutoLockTimer = useCallback(() => {
    ...
    // Only set new timer if device is not currently locked
    if (!isLocked) {
      const newTimer = setTimeout(() => {
        // Double-check that enough time has actually passed
        const timeSinceLastActivity = Date.now() - lastActivityRef.current;
        if (timeSinceLastActivity >= 59000) { // 59 seconds to account for small delays
          console.log("Auto-locking device after 60 seconds of inactivity");
          setIsLocked(true);
          
          // Update hardware lock state
          if (encoderConnected) {
            toggleLock().catch(err => console.error('Failed to toggle hardware lock state:', err));
          }
        } else {
          // If not enough time has passed, reschedule
          console.log(`Not enough time passed (${timeSinceLastActivity}ms), rescheduling auto-lock`);
          const remainingTime = 60000 - timeSinceLastActivity;
          ...
          setAutoLockTimer(rescheduleTimer as unknown as NodeJS.Timeout);
        }
      }, 60000); // 60 seconds
      
      setAutoLockTimer(newTimer as unknown as NodeJS.Timeout);
      console.log("Auto-lock timer reset - will lock in 60 seconds");
    }
  }, [autoLockTimer, encoderConnected, isLocked]);

  // Function to track any user activity
  const trackActivity = useCallback((source: string) => {
    console.log(`Activity detected from: ${source}`);
    lastActivityRef.current = Date.now();
    
    // Only reset timer if device is unlocked
    if (!isLocked) {
      resetAutoLockTimer();
    }
  ...
  const handleReset = useCallback(() => {
    ...
    console.log("All values reset to initial state");
  }, [encoderConnected, trackActivity]);

  // Handle mode navigation - memoized with useCallback
  const handleModeNavigation = useCallback((direction: 'up' | 'down') => {
    trackActivity(`mode navigation ${direction}`);
  ...
  }, [trackActivity, isLocked, showDDDSettings, showDOOSettings, selectedDDDSetting, pendingModeIndex, modes.length, handleLockError, encoderConnected, dddSettings.aSensitivity, dddSettings.vSensitivity]);

  const handleLeftArrowPress = useCallback(() => {
    trackActivity('left arrow press');
  ...
  }, [trackActivity, isLocked, showDDDSettings, showVVISettings, showDOOSettings, showAAISettings, pendingModeIndex, modes, showAsyncMessage, encoderConnected, handleLockError, dddSettings.aSensitivity, vviSensitivity, aaiSensitivity]);
...
}, [encoderConnected, showDDDSettings, showVVISettings, selectedDDDSetting, dddSettings.aSensitivity, dddSettings.vSensitivity, vviSensitivity]);
...
useEffect(() => {
  ...
  useEffect(() => {
    ...
    const stopPolling = startEncoderPolling(
      // Control values callback
      (data: EncoderControlData) => {
        // Track activity from hardware encoders
        ...
        // Update local state from hardware and track activity
        if (data.rate !== undefined && Math.abs(data.rate - rate) > 0.1) {
          setRate(data.rate);
        ...
        // Track hardware encoder activity
        if (activityDetected) {
          trackActivity('hardware encoder');
        }

        // handles lock state changes - with race condition prevention
        if (data.locked !== undefined && data.locked !== isLocked) {
          // Don't update if we're already changing lock state
          if (!lockStateChangingRef.current) {
            console.log(`Lock state change from hardware polling: ${data.locked}`);
            setIsLocked(data.locked);
            if (data.locked) {
              // If device just locked, clear any auto-lock timer
              if (autoLockTimer) {
              ...
              // If device just unlocked, start auto-lock timer
              setTimeout(() => {
                resetAutoLockTimer();
              }, 300);
            }
      ...
      (status) => {
        ...
        // Track encoder activity more specifically
        if (status.encoder_active) {
          console.log("Encoder activity detected from status - resetting auto-lock timer");
          trackActivity('encoder activity flag');
        }
  ...
  }, [encoderConnected, rate, aOutput, vOutput, vviSensitivity, localControlActive, autoLockTimer, isLocked, isControlsLocked, selectedModeIndex, trackActivity]);

  // Initialize auto-lock timer when component mounts
  useEffect(() => {
    if (!isLocked) {
      resetAutoLockTimer();
    }
  ...
  // Handle VVI sensitivity change
  const handleVVISensitivityChange = (value: number) => {
    trackActivity('VVI sensitivity change');
  ...
  // Handle AAI sensitivity change
  const handleAAISensitivityChange = (value: number) => {
    trackActivity('AAI sensitivity change');
  ...
  // Activate emergency mode
  const handleEmergencyMode = useCallback(() => {
    trackActivity('emergency mode toggle');
  ...
  useEffect(() => {
    const handleHardwareUpButtonPress = () => {
      console.log("Hardware up button press detected");
      trackActivity('hardware up button');
      handleModeNavigation('up');
    };

    const handleHardwareDownButtonPress = () => {
      console.log("Hardware down button press detected");
      trackActivity('hardware down button');
      handleModeNavigation('down');
    };

    const handleHardwareLeftButtonPress = () => {
      console.log("Hardware left button press detected");
      trackActivity('hardware left button');
      handleLeftArrowPress();
    };

    const handleHardwareEmergencyButtonPress = () => {
      console.log("Hardware emergency button press detected");
      trackActivity('hardware emergency button');
      handleEmergencyMode();
    };
  ...
  const handleLockToggle = async () => {
    ...
    if (now - lastLockToggleRef.current < 1000) {
      console.log("Ignoring rapid lock toggle to prevent race condition");
      return;
    }
    lastLockToggleRef.current = now;
    
    // Prevent multiple concurrent lock state changes
    if (lockStateChangingRef.current) {
      console.log("Lock state change already in progress, ignoring");
      return;
    }
    ...
    try {
      ...
      // Clear auto-lock timer when manually locking/unlocking
      if (autoLockTimer) {
        clearTimeout(autoLockTimer);
        setAutoLockTimer(null);
      }
      
      // Update the hardware lock state
      if (encoderConnected) {
        try {
          const hardwareLockState = await toggleLock();
          if (hardwareLockState !== null && hardwareLockState !== newLockState) {
            console.log("Hardware lock state doesn't match UI state, correcting UI");
            setIsLocked(hardwareLockState);
            
            // Start timer if hardware says we're unlocked
            if (!hardwareLockState) {
              setTimeout(() => {
                resetAutoLockTimer();
              }, 200);
            }
          } else {
            // States match, start timer if unlocking
            if (!newLockState) {
              setTimeout(() => {
                resetAutoLockTimer();
              }, 200);
        ...
        // No hardware connection, just manage timer locally
        if (!newLockState) {
          setTimeout(() => {
            resetAutoLockTimer();
          }, 200);
      ...
      // Always clear the changing flag after a delay
      setTimeout(() => {
  ...
  // Handle lock state changes from hardware with enhanced synchronization
  useEffect(() => {
    if (!encoderConnected) return;
    
    const checkLockState = async () => {
      // Don't check if we're in the middle of changing lock state
      if (lockStateChangingRef.current) {
        return;
      }
      ...
      try {
        const lockState = await getLockState();
        if (lockState !== null && lockState !== isLocked) {
          console.log(`Hardware lock state changed: ${lockState}, updating UI`);
          
          // Prevent race condition by setting the changing flag
          lockStateChangingRef.current = true;
          ...
          // If device just unlocked, start new auto-lock timer
          if (!lockState) {
            setTimeout(() => {
              resetAutoLockTimer();
            }, 300);
          ...
          // Clear the changing flag after state settles
          setTimeout(() => {
            lockStateChangingRef.current = false;
          }, 1000);
        }
    ...
    // Initial check with delay to let component settle
    setTimeout(checkLockState, 1000);
    
    // Check every 1000ms for lock state changes (reduced frequency to prevent fighting)
        ...
        <div className="grid grid-cols-2 gap-4">
          {modes.map((mode, index) => (
            <button
              key={mode}
              onClick={() => {
                handleUIClick(`mode button ${mode}`);
                // Special handling for DOO mode - show emergency content
                if (mode === 'DOO') {
                  handleEmergencyMode();
                  return;
  ...
  return (
    <div className="max-w-2xl mx-auto p-8 bg-gray-50 min-h-screen">
      ...
      <button
        onClick={() => {
          handleUIClick('emergency mode button');
          handleEmergencyMode();
        }}
        className="w-full mb-4 bg-red-500 text-white py-2 px-4 rounded-xl hover:bg-red-600 transition-colors"
      >
      ...
      <div 
        ...
        onClick={() => handleUIClick('main controls area')}
      >
      ...
      <div className="flex gap-4">
        <div 
          className="bg-white rounded-3xl shadow-sm p-6 flex-1" 
          style={{ minHeight: '300px' }}
          onClick={() => handleUIClick('mode panel')}
        >
        ...
        <div className="flex flex-col gap-8 my-3">
          ...
          <button 
            onClick={() => {
              handleUIClick('left arrow button');
              handleLeftArrowPress();
            }}
            className="w-10 h-10 rounded-xl bg-white shadow-sm flex items-center justify-center hover:bg-gray-50"
          >
          ...
          <button 
            onClick={() => {
              handleUIClick('down arrow button');
              handleModeNavigation('down');
            }}
            className="w-10 h-10 rounded-xl bg-white shadow-sm flex items-center justify-center hover:bg-gray-50"
          >
